...
def check_rules(prompt: str) -> dict:
    ...
    with tracer.start_as_current_span("check_rules") as span:
        result = analyze_prompt(prompt)
        span.set_attribute("risk_score", result["risk_score"])
        span.set_attribute("triggered_rules", str([r["rule"] for r in result["triggered_rules"]]))
        span.set_attribute("is_suspicious", result["is_suspicious"])
...
def block_request(reason: str) -> str:
    """Блокируем запрос"""
    with tracer.start_as_current_span("block_request") as span:
...
def run_agent(user_prompt: str) -> dict:
    ...
    with tracer.start_as_current_span("agent_run") as span:
        span.set_attribute("user_prompt", user_prompt[:200])
        ...
        if analysis["risk_score"] >= 50:
            ...
            span.set_attribute("block_reason", "risk_score >= 50")
